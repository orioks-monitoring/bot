...
def select_count_notify_settings_marks() -> int:
    db = sqlite3.connect(config.PATH_TO_DB)
    sql = db.cursor()
    with open(os.path.join(config.PATH_TO_SQL_FOLDER, 'select_count_notify_settings_marks.sql'), 'r') as sql_file:
        sql_script = sql_file.read()
    count_notify_settings_marks = sql.execute(sql_script).fetchone()
