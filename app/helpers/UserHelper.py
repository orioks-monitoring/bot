...
class UserHelper:
    ...
    def __get_user_by_telegram_id(user_telegram_id: int) -> UserStatus:
        ...
        if user is None:
            raise DatabaseException(f'User with telegram id {user_telegram_id} not found in database')

        return user

    @staticmethod
    def get_user_settings_by_telegram_id(user_telegram_id: int) -> UserNotifySettings:
        user_notify_settings = UserNotifySettings.find_one(user_telegram_id=user_telegram_id)
        if user_notify_settings is None:
            raise DatabaseException(f'Settings of user with telegram id {user_telegram_id} not found in database')
    ...
    def create_user_if_not_exist(user_telegram_id: int) -> None:
        ...
        existed_user_settings = UserNotifySettings.find_one(user_telegram_id=user_telegram_id)
    ...
    @staticmethod
    def update_authorization_status(user_telegram_id: int, is_authenticated: bool) -> None:
        user = UserHelper.__get_user_by_telegram_id(user_telegram_id=user_telegram_id)
    ...
    def reset_notification_settings(user_telegram_id: int) -> None:
        user_settings = UserHelper.get_user_settings_by_telegram_id(user_telegram_id=user_telegram_id)
        user_settings.fill(user_telegram_id=user_telegram_id)
        user_settings.save()
    ...
    def update_notification_settings(user_telegram_id: int, setting_name: str) -> None:
        ...
        if getattr(user_settings, setting_name) is None:
            raise DatabaseException(f'Setting with name {setting_name} for user with id {user_telegram_id} not found')

        setattr(user_settings, setting_name, not bool(getattr(user_settings, setting_name)))
        user_settings.save()
